...
public class ActorModel extends Database {
    ...
    public ResultSet getCollection() throws Exception {
        try {
            PreparedStatement = Connection.prepareStatement("SELECT actor.id, person_id, person.name, surname, birthday, person.picture, country_id, country.picture AS country_picture, country.name AS country_name  FROM actor LEFT OUTER JOIN person ON actor.person_id = person.id LEFT OUTER JOIN country ON person.country_id = country.id");
        } catch (SQLException e) {
            LogService.insert(e);
        }
    ...
    public ResultSet getCollection(int movie_id) throws Exception {
        try {
            PreparedStatement = Connection.prepareStatement("SELECT actor.id, person_id, person.name, surname, birthday, person.picture, country_id, country.picture AS country_picture, country.name AS country_name  FROM actor LEFT OUTER JOIN person ON actor.person_id = person.id LEFT OUTER JOIN country ON person.country_id = country.id LEFT OUTER JOIN movie_actor ON actor.id = movie_actor.actor_id WHERE movie_actor.movie_id = ?");
